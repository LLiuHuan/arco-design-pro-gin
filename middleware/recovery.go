		...
		defer func() {
			if err := recover(); err != nil {
				...
				if brokenPipe {
					...
					response.ResultAll(http.StatusInternalServerError, gin.H{}, "系统内部错误-1", c)
					c.Abort()
					return
				}
				...
				if stack {
					...
					response.ResultAll(http.StatusInternalServerError, gin.H{}, "系统内部错误-2", c)
				} else {
					global.AdpLog.Error("[Recovery from panic]",
						zap.Any("error", err),
						zap.String("request", string(httpRequest)),
					)
					response.ResultAll(http.StatusInternalServerError, gin.H{}, "系统内部错误-3", c)
				}
