...
func (f *fileMysql) CheckDataExist() bool {
	if errors.Is(global.AdpDb.Where("`name` = ? AND `key` = ?", "logo.png", "1587973709logo.png").First(&example.ExaFileUploadAndDownload{}).Error, gorm.ErrRecordNotFound) {
		return false
	}
	return true
}
